...
export async function PUT(request: NextRequest) {
  ...
  if (!token) {
    console.error(
      '[PUT /api/organization/[id]/funnel/[funnelId]/assistant] No token received from __session cookie.'
    );
    return NextResponse.json(
      { error: 'Authentication failed.' },
      { status: 401 }
  ...
  console.log(
    '[PUT /api/organization/[id]/funnel/[funnelId]/assistant] Token received from __session cookie, updating assistant.'
  );

  try {
    const body = await request.json();
    console.log(
      '[PUT /api/organization/[id]/funnel/[funnelId]/assistant] Request body:',
      body
    );
    ...
    console.log(
      `[PUT /api/organization/[id]/funnel/[funnelId]/assistant] Response status: ${response.status}`
    );
    ...
    if (!response.ok) {
      ...
      try {
        const errorData = await response.json();
        console.error(
          '[PUT /api/organization/[id]/funnel/[funnelId]/assistant] API error response:',
          errorData
        );
    ...
    const data = await response.json();
    console.log(
      '[PUT /api/organization/[id]/funnel/[funnelId]/assistant] Successfully updated assistant:',
      data
    );
    return NextResponse.json(data);
  } catch (error) {
    console.error(
      '[PUT /api/organization/[id]/funnel/[funnelId]/assistant] Unexpected error:',
      error
    );
    return NextResponse.json(
